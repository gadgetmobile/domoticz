...
// TODO: regulator state (on/off)
static std::string with_desired_temperature(int temperature_x100) {
  Json::Value form;
  form["heat"]["desiredTemp"] = temperature_x100;
  return test_helper::compact_json(form);
}

static std::string with_state(int value) {
  Json::Value form;
  form["heat"]["state"] = value;
  return test_helper::compact_json(form);
}
