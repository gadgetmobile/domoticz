...
static std::string with_relay_state(uint8_t relay_number, std::string state) {
  BOOST_ASSERT(state == "on" || state == "off");

  Json::Value form;
  form["relays"][0]["relay"] = relay_number;
  form["relays"][0]["state"] = state == "on" ? 1 : 0;
