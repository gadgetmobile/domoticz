...
Json::Value blebox::widgets::color_switch::post_raw_rfx(
  ...
  case Color_LedOn: {
    ...
    uint8_t level = data->value;
    col.ww = brightness_from_level(level);
  ...
  case Color_SetColor: { // 10
    ...
    col.ww = brightness_from_level(data->value);
  ...
  case Color_SetBrightnessLevel: { // 15
    ...
    uint8_t g = color.g;
    uint8_t b = color.b;
    uint32_t new_col = uint32_t(r << 24) | uint32_t(g << 16) |
                       uint32_t(b << 8) |
                       uint32_t(brightness_from_level(data->value));
