...
static void HSPtoRGB(double H, double S, double P, uint8_t *r, uint8_t *g,
                     uint8_t *b) {
  ...
  *r = uint8_t(std::round(R * 255));
  *g = uint8_t(std::round(G * 255));
  *b = uint8_t(std::round(B * 255));
}

// TODO: replace with HSLuv
static void RGBtoHSP(uint8_t r, uint8_t g, uint8_t b, double *H, double *S,
                     double *P) {
  //  Calculate the Perceived brightness.
  const double Pr = .299;
  const double Pg = .587;
  const double Pb = .114;
  *P = sqrt(r * r * Pr + g * g * Pg + b * b * Pb) / 255.0;
